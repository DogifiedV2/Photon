...
| PT-002 | Likely bug | Blocker | Client editor command open path | User confirmed `/photon_editor` executes/exists but opens no menu and produced no useful log output in the running client. Existing command opened the screen synchronously from command execution, which can be immediately overridden by chat/command screen closing. | Added explicit Photon log/chat debug messages for client command registration and `/photon_editor` execution, and schedules editor screen creation on the next Minecraft client task with try/catch logging. | `:photon-forge:compileJava` passed on 2026-05-16. Requires rerun client and manual `/photon_editor` test to verify editor open. |
