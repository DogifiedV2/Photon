...
public class BeamParticle implements IParticle {
    ...
    public BeamParticle(IParticleEmitter emitter, BeamConfig config, Random randomSource) {
    ...
    public void render(@Nonnull VertexConsumer pBuffer, @Nonnull Camera camera, float partialTicks) {
        if (delay <= 0 && this.emitter.isVisible()) {
            var cameraPos = new Vector3f((float) camera.getPosition().x, (float) camera.getPosition().y, (float) camera.getPosition().z);
