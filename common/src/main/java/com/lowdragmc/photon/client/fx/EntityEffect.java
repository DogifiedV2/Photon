...
public class EntityEffect extends FXEffect {
            ...
            if (autoRotate != AutoRotate.NONE) {
                switch (autoRotate) {
                    ...
                    case XROT -> {
                        var newRotation = new Quaternionf(rotation).rotateXYZ(
                                0,
                                Math.toRadians(-90 - entity.getViewYRot(0)),
                                0
                        );
                        runtime.root.updateRotation(newRotation);
                    }
    ...
    public void start() {
        ...
        root.updatePos(new Vector3f((float) entity.getPosition(0).x, (float) entity.getPosition(0).y, (float) entity.getPosition(0).z).add(offset.x, offset.y, offset.z));
