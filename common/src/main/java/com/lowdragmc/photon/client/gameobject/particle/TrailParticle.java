...
public class TrailParticle implements IParticle {
    ...
    public TrailParticle(IParticleEmitter emitter, TrailConfig config, Random randomSource) {
    ...
    public void render(@Nonnull VertexConsumer pBuffer, Camera pRenderInfo, float partialTicks) {
        if (delay <= 0 && this.emitter.isVisible()) {
            tails.renderInternal(pBuffer, partialTicks, new Vector3f((float) pRenderInfo.getPosition().x, (float) pRenderInfo.getPosition().y, (float) pRenderInfo.getPosition().z), getRealColor(partialTicks), getRealLight(partialTicks));
        }
    }
