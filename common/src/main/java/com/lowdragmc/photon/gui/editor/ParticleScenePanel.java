...
public class ParticleScenePanel extends WidgetGroup {
    ...
    public void resetScene() {
        ...
        scene.getRenderer().setCameraLookAt(new com.mojang.math.Vector3f(center.x, center.y, center.z), scene.camZoom(), Math.toRadians(scene.getRotationPitch()), Math.toRadians(scene.getRotationYaw()));
    }
        ...
        if (project.isDraggable() && fxObjectsList != null) {
            ...
            if (selected != null) {
                ...
                if (draggingSelected) {
                    ...
                    ray = ray.toInfinite();
                    var result = aabb.clip(new Vec3(ray.startPos().x, ray.startPos().y, ray.startPos().z), new Vec3(ray.endPos().x, ray.endPos().y, ray.endPos().z));
                    if (result.isPresent()) {
                        hoverSelected = true;
                    }
                }
